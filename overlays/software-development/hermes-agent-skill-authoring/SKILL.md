...
2. **In-repo (this skill is about this case):** `<hermes-repo>/skills/<category>/<name>/SKILL.md` — shipped with the package. Use `write_file`; commit only when the user asks. `skill_manage(action='create')` does NOT target this tree.
...
4. **Validate locally**:
   ...
   ```
5. **Run behavior and structure verification.** Leave the result in the working diff unless the user asks for a commit.
...
- Treat in-repo skills as source and show the complete diff before any user-approved commit.

## Behavior-First Skill Development

Skills are behavior-shaping code. Use RED–GREEN–REFACTOR:

1. **RED:** run representative prompts without the proposed wording and record the undesired behavior.
2. **GREEN:** make the smallest instruction change that reliably corrects that behavior.
3. **REFACTOR:** remove duplicated or no-op prose while keeping the behavior green.

For discipline or pressure-sensitive skills, test at least five independent samples across urgency, authority, sunk-cost, and fatigue pressure. Include a no-guidance control. Judge transcripts and actions, not whether the model can quote the skill.

Use [`references/testing-skills-with-subagents.md`](references/testing-skills-with-subagents.md) for the upstream pressure-testing method and [`references/persuasion-principles.md`](references/persuasion-principles.md) when hard-gate wording needs evaluation. Anthropic-specific advice in [`references/anthropic-best-practices.md`](references/anthropic-best-practices.md) is supporting context, not Hermes runtime policy.
...
- [ ] Baseline and guided behavior evidence recorded where behavior changed
- [ ] Complete diff reviewed; commit left to the user's explicit decision
