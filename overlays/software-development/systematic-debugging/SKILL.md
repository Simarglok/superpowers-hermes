...
Load only when the debugging path needs them:

- [`references/root-cause-tracing.md`](references/root-cause-tracing.md) — trace bad values backward across call boundaries;
- [`references/defense-in-depth.md`](references/defense-in-depth.md) — add validation at multiple layers after the root cause is known;
- [`references/condition-based-waiting.md`](references/condition-based-waiting.md) — replace arbitrary sleeps with observable conditions;
- [`references/condition-based-waiting-example.ts`](references/condition-based-waiting-example.ts) — TypeScript example;
- `scripts/find-polluter.sh` — isolate a test that contaminates shared state. Inspect it before execution and run it only in a disposable test context.
